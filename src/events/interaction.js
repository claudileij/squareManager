...
module.exports = class extends Event {
    ...
    run = async (interaction) => {
        ...
        if (interaction.isCommand()) {
            const commandHandler = this.client.commands.find(c => c.name === interaction.commandName)
            if (commandHandler) { commandHandler.run(interaction) }
        }
        ...
        if (interaction.isStringSelectMenu()) {
            const selectHandler = this.client.selectMenus.find(s => s.name == interaction.customId)
            if (selectHandler) { selectHandler.run(interaction) }
        }
